        ...
        for feat in geojson['features']:
            if feat['properties']['NAME_1'] == row['NAME_1']:
                ...
                folium.Marker(
                    [lat, lon],
                    icon=folium.DivIcon(
                        html=f'<div style="font-size:11px;font-weight:bold;background:white;padding:2px 4px;border-radius:3px">{int(row["Jumlah Responden"])}</div>'
                    )
                ).add_to(m)
                break
...
with tab1:
    ...
    def kpi_card(col, label, value, sub, color, icon=""):
        with col:
            ...
            <div style="background:{WHITE};border-radius:14px;padding:18px 20px;
                ...
                <p style="font-size:1.5rem;font-weight:700;color:#1a1a2e;
    ...
    with col_nps:
        ...
        if nps_col:
            pct_passive = passives / n_total * 100 if n_total > 0 else 0
        else:
